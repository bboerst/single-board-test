...
void save_fixed_FreqAndCoreNum_toPIC()
{
	...
	for (i = 0; i < BITMAIN_MAX_CHAIN_NUM; i++)
	{
		...
		do
		{
			...
			if (memcmp(buf, chain_pic_buf[i], 128) != 0 || memcmp(badcore_buf, pic_badcore_num[i], 64) != 0)
			{
				sprintf(logstr, "Error: flash write error! on Chain[%d]\n", i);
				writeLogFile(logstr);
	...
	for (i = 0; i < BITMAIN_MAX_CHAIN_NUM; i++)
	{
		...
		do
		{
			...
			if (memcmp(buf, chain_pic_buf[i], 128) != 0 || memcmp(badcore_buf, pic_badcore_num[i], 64) != 0)
			{
				...
				while (1)
					sleep(1); // we need wait here forever!!! for DEBUG
#endif
			}
			else
			{
		...
		)
		{
			...
			if (cgpu.chain_asic_num[i] != ASIC_NUM)
			{
				...
				if (isChipNumOK_Once)
					...
					sprintf(search_failed_info, "J%d:2", i + 1);
	...
	for (i = 0; i < BITMAIN_MAX_CHAIN_NUM; i++)
	{
		...
		if (last_all_pass(i))
		{
			...
			if (testModeOKCounter[i] >= TEST_MODE_OK_NUM)
				...
				sprintf(logstr, "Test Patten on chain[%d]: OK! board_temp=%d testing...\n", i, board_temp[i]);
			writeLogFile(logstr);
		...
		else
		{
			if (testModeOKCounter[i] >= TEST_MODE_OK_NUM)
				...
				sprintf(logstr, "Test Patten on chain[%d]: FAILED! board_temp=%d testing...\n", i, board_temp[i]);
			writeLogFile(logstr);
				...
				if (fpga_version >= 0xE)
				{
					...
					for (j = 0; j < ASIC_NUM; j++)
					{
						if (j % 2)
							...
							chain_badcore_num[i][j] = (chain_pic_buf[new_T9_PLUS_chainIndex][7 + new_T9_PLUS_chainOffset * 31 + 22 + (j / 2)] >> 4) & 0x0f;
				...
				else
				{
					...
					for (j = 0; j < ASIC_NUM; j++)
					{
						if (j % 2)
							...
							chain_badcore_num[i][j] = (chain_pic_buf[((i / 3) * 3)][7 + (i % 3) * 31 + 22 + (j / 2)] >> 4) & 0x0f;
				...
				if (badcore_buf[0] == BADCORE_MAGIC)
				{
					...
					for (j = 0; j < ASIC_NUM; j++)
					{
						if (j % 2)
							...
							chain_badcore_num[i][j] = (badcore_buf[(j / 2) * 2 + 1] >> 4) & 0x0f;
...
int main(int argc, char *argv[])
{
	...
	if (conf.force_freq)
	{
		...
		if (is32xPattenReady)
		{
			...
			TEST_MODE_OK_NUM = TEST_MODE_OK_NUM_8X;
